...
const SWAGGER_FILE_PATH = process.env.SWAGGER_FILE_PATH || path.resolve("src", "swagger.json");
...
const loadSwaggerDocument = async () => {
    ...
    try {
        ...
        if (error.code === 'ENOENT') {
            console.warn(`Swagger file not found at path: ${SWAGGER_FILE_PATH}. Using default documentation.`);
        } else {
            console.warn(`Failed to load swagger.json from path: ${SWAGGER_FILE_PATH}. Using default documentation.`);
            console.error(error);
        }
